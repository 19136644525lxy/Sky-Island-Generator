...
一个 Minecraft 1.20.1 Forge 模组，添加"空岛"世界类型。在创建世界时选择该类型后，三个维度均会生成小型起始平台，周围是虚空，同时保留原版特殊建筑（末地传送门、古城传送门、刷怪笼等）和生物群系的正常生成。
...
### 下界
- 在 (8, 64, 8) 处生成 5×5×3 下界岩平台
- 下界要塞仅生成烈焰人刷怪笼房间（MonsterThrone）
- 堡垒遗迹保持原版生成不变
- 多生物群系正常分布

### 末地
- 保留原版黑曜石平台（玩家进入末地时自动生成）
...
### 世界类型注册
通过数据驱动的 `world_preset` JSON 文件定义空岛世界预设，并追加到原版 `normal` 世界类型标签，无需修改创建世界界面代码。
...
模组配置文件位于 `config/skyisland-common.toml`：

| 配置项 | 说明 | 默认值 |
|--------|------|--------|
| `overworld.platformX/Y/Z` | 主世界平台坐标 | 0 / 64 / 0 |
| `overworld.platformSize` | 主世界平台边长 | 5 |
| `nether.platformX/Y/Z` | 下界平台坐标 | 0 / 64 / 0 |
| `nether.platformSize` | 下界平台边长 | 5 |
| `end.preserveObsidianPlatform` | 是否保留末地黑曜石平台 | true |

> **注意**：实际平台坐标由 `world_preset` JSON 文件控制（区块中心 8,64,8），配置文件仅作为注册声明。
